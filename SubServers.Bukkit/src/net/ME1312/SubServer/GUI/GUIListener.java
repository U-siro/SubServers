...
public class GUIListener implements Listener {
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		if ((a == Action.RIGHT_CLICK_AIR || a == Action.RIGHT_CLICK_BLOCK) && is != null && is.getType() == Material.getMaterial(Main.config.getInt("Settings.GUI.Trigger-Item")) && event.getPlayer().hasPermission("SubServer.Command") && Main.config.getBoolean("Settings.GUI.Enabled")) {
							...
							if (!StringUtils.isAlphanumericSpace(Name)) {
								...
								if (Memory == 0) {
									...
									if (Port == 0 || Port > 65535) {
										...
										Main.ServerCreator = new SubServerCreator(Name, Port, new File("./" + Name), new Executable("java -Xmx" + Memory + "M -Djline.terminal=jline.UnsupportedTerminal -Dcom.mojang.eula.agree=true -jar " + Jar), player, Type, Version, Main);
