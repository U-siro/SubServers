            ...
            while ((line = br.readLine()) != null)
            	if (id.equalsIgnoreCase("~Proxy")) {
            		if (log && !line.startsWith(">") && !line.contains("subconf@")) {
            			Main.log.info(Main.lang.getNode("Lang", "Debug", "Server-Logging-Prefix").getString().replace("$Server$", "Proxy") +
            					line.replace(new SimpleDateFormat("HH:mm:ss").format(Calendar.getInstance().getTime()) + " ", ""));
            		}
            	} else {
            		if (log && !line.startsWith(">") && !line.contains("subconf@")) {
            			Main.log.info(Main.lang.getNode("Lang", "Debug", "Server-Logging-Prefix").getString().replace("$Server$", id) + line
